...
import { PageSeoAnalyzeStatusList } from '../interfaces/scraper-status.enum';
...
type ModelFields = keyof typeof RefEnum | keyof typeof Prisma.PageSeoAnalyzeScalarFieldEnum;
...
/*                                    SHOW                                    */
/* -------------------------------------------------------------------------- */
export const showPropsBuilder = (item: PageSeoAnalyze): Array<ShowProps> => {
...
/* --------------------------------- ACTIONS -------------------------------- */
export const showActionBuilder = (item: PageSeoAnalyze): Array<ShowAction> => {
...
export const tablePropsBuilder = (availableActions: Array<AvailableAction>): ModifiedTableProps => {
  const tableProps: ModifiedTableProps = {
    ...
    modelTitle: 'آنالیز صفحات',
    columns: [
      { id: 10, title: 'url', key: 'url', cellType: 'link', optionalClass: 'ltr' },
      {
        id: 20, title: 'طول عنوان', key: 'meta_title_length', cellType: 'string',
        conditions: [
          { operator: 'between', target: [SeoParamsLimit.MIN_TITLE_LENGTH, SeoParamsLimit.MAX_TITLE_LENGTH], className: 'bg-success' },
          { operator: 'gt', target: SeoParamsLimit.MAX_TITLE_LENGTH, className: 'bg-danger' },
          { operator: 'lt', target: SeoParamsLimit.MIN_TITLE_LENGTH, className: 'bg-danger' },
        ]

      },
      {
        id: 30, title: 'طول توضیحات', key: 'meta_description_length', cellType: 'string', conditions: [
          { operator: 'between', target: [SeoParamsLimit.MIN_DESCRIPTION_LENGTH, SeoParamsLimit.MAX_DESCRIPTION_LENGTH], className: 'bg-success' },
          { operator: 'gt', target: SeoParamsLimit.MAX_DESCRIPTION_LENGTH, className: 'bg-danger' },
          { operator: 'lt', target: SeoParamsLimit.MIN_DESCRIPTION_LENGTH, className: 'bg-danger' },
        ]
      },
      {
        id: 40, title: 'تعداد H1', key: 'h1_count', cellType: 'string', conditions: [
          { operator: 'gt', target: 1, className: 'bg-danger' }
        ]
      },
      { id: 50, title: 'تگ های H1', key: 'h1_array', cellType: 'arrayOfStrings', optionalClass: 'text-xs p-0' },
      {
        id: 60, title: 'تعداد تصاویر بدون alt', key: 'images_count', cellType: 'string',
        conditions: [{
          operator: 'gte', target: 1, className: 'bg-danger'
        }]
      },
      { id: 71, title: 'تعداد اسکیماها', key: 'schemas_count', cellType: 'string' },
      { id: 70, title: 'canonical', key: 'canonical', cellType: 'boolean' },
      { id: 73, title: 'وضعیت', key: 'scraper_flag', cellType: 'enum', enumList: PageSeoAnalyzeStatusList },
      /* ---------------------------------- date ---------------------------------- */
...
export const filterPropsBuilder = (): ModifiedFilterProps[] => {
  const filterProps: Array<ModifiedFilterProps> = [
    ...
    { state: '', type: 'break' },
    {
      title: 'عنوان نیاز به اصلاح؟',
      state: 'title_issue',
      type: 'switch',
    },
    {
      title: 'توضیحات نیاز به اصلاح؟',
      state: 'description_issue',
      type: 'switch',
    },
    {
      title: 'مشکل در H1',
