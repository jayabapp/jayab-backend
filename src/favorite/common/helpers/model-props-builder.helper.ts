import { AccessControlList, Favorite, Prisma } from '@prisma/client';
import {
  AvailableAction,
  Column,
  CreateProps,
...
type ModelFields = keyof typeof RefEnum | keyof typeof Prisma.FavoriteScalarFieldEnum;
...
/*                                    SHOW                                    */
/* -------------------------------------------------------------------------- */
export const showPropsBuilder = (item: Favorite): Array<ShowProps> => {
...
/* --------------------------------- ACTIONS -------------------------------- */
export const showActionBuilder = (item: Favorite): Array<ShowAction> => {
...
export const filterPropsBuilder = (): ModifiedFilterProps[] => {
  const filterProps: Array<ModifiedFilterProps> = [
    ...
    { id: 10, title: 'عنوان', key: 'title', type: 'string', operators: [operators.contains] },
    /* --------------------------------- SELECT --------------------------------- */
    // {id: 20,title: 'دسته بندی',key: items.category_key,type: 'select',selectItems: ParentCategoriesList,disabled: true,},
  ];
