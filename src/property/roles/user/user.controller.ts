...
export class PropertyUserController {
  ...
  @Post()
  async create(@Body() dto: CreatePropertyUserDto): Promise<SuccessResponseArgs> {
    const result = await this.propertyUserService.create(dto);
  ...
  async update(
    @Param("propertyId", ParseIntPipe) propertyId: number,
    @Body() dto: UpdatePropertyUserDto
  ): Promise<SuccessResponseArgs> {
    await this.propertyUserService.findOne(propertyId);
    const result = await this.propertyUserService.update(propertyId, dto);
  ...
  // @Delete(':propertyId')
  // async remove(@Param('propertyId', ParseIntPipe) propertyId: number): Promise<SuccessResponseArgs> {
  //   await this.propertyUserService.findOne(propertyId);
  //   await this.propertyUserService.remove(propertyId);
